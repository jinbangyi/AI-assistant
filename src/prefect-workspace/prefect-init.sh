...
# Create work pool for hyperliquid workflows
echo "Creating work pool 'hyperliquid-pool'..."
prefect work-pool create hyperliquid-pool --type process 2>/dev/null || \
  (prefect work-pool inspect hyperliquid-pool >/dev/null 2>&1 && echo "Work pool already exists")
